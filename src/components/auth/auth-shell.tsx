      ...
      <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-glow lg:p-8">
        ...
        <div className="relative space-y-6">
          <div className="flex items-center gap-3">
            <div className="grid h-14 w-14 place-items-center overflow-hidden rounded-2xl border border-white/10 bg-black/30">
              <Image src="/bytebit-logo1.png" alt={`${appName} logo`} width={56} height={56} className="h-14 w-14 object-cover" />
            </div>
            <div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              { label: "Secure JWT", icon: ShieldCheck },
              { label: "Fast checkout", icon: UtensilsCrossed },
              { label: "Polished UX", icon: Sparkles },
            ].map((item) => {
